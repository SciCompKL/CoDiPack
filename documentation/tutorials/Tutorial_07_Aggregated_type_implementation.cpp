...
template<typename> // Template argument not used, we use the direct type specification of Vector4.
struct ScalarVectorMultiplicationOperation : public codi::BinaryOperation<Vector4> {

    using Jacobian = std::tuple<Vector4Transpose, Number>;

    static CODI_INLINE Vector4 primal(Number const& s, Vector4 const& v) {
      return s * v;
    }

    static CODI_INLINE Vector4Transpose gradientA(Number const& s, Vector4 const& v, Vector4 const& result) {
      return v.transpose();
    }
...
codi::BinaryExpression<Vector4, ArgS, ArgV, ScalarVectorMultiplicationOperation> operator*(
    codi::ExpressionInterface<Number, ArgS> const& s,
    codi::ExpressionInterface<Vector4, ArgV> const& v) {
  return codi::BinaryExpression<Vector4, ArgS, ArgV, ScalarVectorMultiplicationOperation>(s.cast(), v.cast());
}

// Operation for vector + vector.
template<typename> // Template argument not used, we use the direct type specification of Vector4.
struct VectorAdditionOperation : public codi::BinaryOperation<Vector4> {

    using Jacobian = std::tuple<Number, Number>;
...
codi::BinaryExpression<Vector4, ArgV1, ArgV2, VectorAdditionOperation> operator+(
    codi::ExpressionInterface<Vector4, ArgV1> const& v1,
    codi::ExpressionInterface<Vector4, ArgV2> const& v2) {
  return codi::BinaryExpression<Vector4, ArgV1, ArgV2, VectorAdditionOperation>(v1.cast(), v2.cast());
}
//! [Operation implementation]
...
template<typename> // Template argument not used, we use the direct type specification of Vector4.
struct VectorNormOperation : public codi::UnaryOperation<Number> {

    using Jacobian = Vector4;
...
struct codi::ExpressionMemberOperations<Vector4, Impl> {
  public:
    ...
    auto norm() const {
      return codi::UnaryExpression<Number, Impl, VectorNormOperation>(this->cast());
    }
