...
codi::InternalStatementRecordingInterface. The first one defines the basic access to the adjoint variables. The second
...
In the following the implementation will now be disused step by step.
...
each stream consists only of one data item per entry in the stream. The `OperatorCode` type is an enum class and will be
covered in more detail later. The interesting thing is, that the type definitions are nested. In order to define
the data stream for the identifier data, the data stream for the operator data is provided as the nested stream.
The design decission for this nesting will become clear when we discuss the reverse interpretation of the data.
...
generated. Therefor we only keep track of the largest one and define the adjoint vector as a standard vector:
...
generation of a new identifiers is also quite simple but standardized in `generateIdentifier`:
...
left hand side identifier from the data stream in a linear index management scheme would do this.) In the
...
The identifiers for each value are stored in the AD type provided by CoDiPack. The initialization of the identifier in
the AD value is done by the function `initIdentifier` required by the codi::InternalStatementRecordingInterface. We
implement an online activity analysis in this tape. Therefore, all identifiers in the AD values can be initialized with
zero. The zero identifier is used in our implementation to track _passive_ values. This are values, that do not depend
on the input values. How this is done is explaned in the next section.
...
the operator is active, that is, it has a non zero identifier. If one argument is active, we store the operation
...
statistics function. All of them do not need to be discussed in detail. Only the reset of the data stream needs to be
...
statement that is differentiated. Since the example is quite simple, it is possible to understand the taping process by
...
d c/d b = -0.1455
-------------------------------------
CoDi Tape Statistics ( JacobiLinearTape )
-------------------------------------
...
of memory and the simple tape requires 200 bytes of memory. In order to keep it simple, no advanced optimization have
been implemented in the SimpleTape. The data management is also quite simple at is not chosen for the minimal memory
footprint.
