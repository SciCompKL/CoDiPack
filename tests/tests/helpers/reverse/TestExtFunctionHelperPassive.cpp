...
void funcCall(NUMBER& z, const NUMBER& w, const NUMBER& v) {
  z = w*v;
}
...
void func(NUMBER* x, NUMBER* y) {
  ...
  for(int i = 1; i < ITER; ++i) {
    ...
    eh.callPassiveFunc(funcCall, w[i], w[i-1], x[1]);
    eh.addOutput(w[i]);

    eh.addToTape(func_reverse, func_forward, func_primal);
  }
