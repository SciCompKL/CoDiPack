...
namespace codi {
  ...
  /**
   * @brief Expression implementation for extended unary operations.
   *
   * @tparam Real  The real type used in the active types.
   * @tparam    A  The expression for the argument of the function.
   * @tparam Impl  Implementation of UnaryExtendedOpInterface.
   */
  template<typename Real, typename A, typename B, template<typename> class Impl>
  struct UnaryExtendedOp : public Expression<Real, UnaryExtendedOp<Real, A, B, Impl> > {
    private:
    ...
    public:
       ...
       */
      explicit UnaryExtendedOp(const Expression<Real, A>& a, B b) :
        a_(a.cast()),
        b_(b),
        result_(Impl<Real>::primal(a.getValue(), b)) {}
    ...
    CODI_INLINE void calcGradient(Data& data) const {
      a_.calcGradient(data, Impl<Real>::gradient(a_.getValue(), b_, result_));
    }
    ...
    CODI_INLINE void calcGradient(Data& data, const Real& multiplier) const {
      a_.calcGradient(data, Impl<Real>::gradient(a_.getValue(), b_, result_)*multiplier);
    }
    ...
    static CODI_INLINE void evalAdjoint(const PRIMAL_SEED_TYPE& seed, const Index* indices, const PassiveReal* constantValues, const Real* primalValues, PRIMAL_ADJOINT_TYPE* adjointValues) {
      B b = Impl<Real>::doubleToArg(constantValues[constantOffset]);
      const Real aPrimal = A::template getValue<Index, offset, constantOffset + 1>(indices, constantValues, primalValues);
      const Real resPrimal = Impl<Real>::primal(aPrimal, b);

      const PRIMAL_SEED_TYPE aJac = Impl<Real>::gradient(aPrimal, b, resPrimal) * seed;
    ...
    static CODI_INLINE Real evalTangent(const Real& seed, GradientValue& lhsAdjoint, const Index* indices, const PassiveReal* constantValues, const Real* primalValues, PRIMAL_ADJOINT_TYPE* adjointValues) {
      ...
      const Real aJac = Impl<Real>::gradient(aPrimal, b, resPrimal) * seed;
    ...
    CODI_INLINE void constantValueAction(Tape& tape, Data data, Func func) const {
      CODI_CALL_MEMBER_FN(tape, func)(data, Impl<Real>::argToDouble(b_));
  ...
  /**
   * @brief Specialization of the TypeTraits for the extended unary operation type.
   *
  ...
  class TypeTraits< UnaryExtendedOp<RealType, A, B, Impl> > {
    public:
      /**
       ...
       */
      typedef const UnaryExtendedOp<RealType, A, B, Impl> StoreType;
       ...
       */
      static const typename TypeTraits<RealType>::PassiveReal getBaseValue(const UnaryExtendedOp<RealType, A, B, Impl>& t) {
