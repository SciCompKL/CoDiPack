...
namespace codi {
  ...
  template<typename _CoDiType, bool _ActiveChecks = true>
  struct Algorithms {
    public:

      using CoDiType = CODI_DECLARE_DEFAULT(_CoDiType, CODI_TEMPLATE(LhsExpressionInterface<double, double, CODI_ANY, CODI_ANY>));
      ...
      using Gradient = typename CoDiType::Gradient;

      using GT = GradientTraits<Gradient>;
      ...
      {
        using GT1st = GT;
        size_t constexpr gradDim1st = GT1st::getVectorSize();
        using GT2nd = GradientTraits<typename Real::Gradient>;
        size_t constexpr gradDim2nd = GT2nd::getVectorSize();

        // Assume tape that was just recorded
        tape.revertPrimals(start);

        for (size_t j = 0; j < inputSize; j += gradDim2nd) {
          setGradient2ndOnIdentifier(tape, j, input, inputSize, typename GT2nd::Data(1.0));
          ...
          for (size_t k = j; k < inputSize; k += gradDim1st) {
            setGradientOnIdentifier(tape, k, input, inputSize, typename GT::Real(1.0));
            ...
            setGradientOnIdentifier(tape, k, input, inputSize, typename GT::Real());
          }

          setGradient2ndOnIdentifier(tape, j, input, inputSize, typename GT2nd::Data());
        }
      }
      ...
      static CODI_INLINE void computeHessianForward(Func func, VecIn& input, VecOut& output, Hes& hes, Jac& jac = StaticDummy<DummyJacobian>::dummy) {
        ...
        for (size_t j = 0; j < input.size(); j += gradDim2nd) {
          ...
          for (size_t k = j; k < input.size(); k += gradDim1st) {
            ...
            setGradientOnCoDiValue(tape, k, input.data(), input.size(), typename GT::Real());
          }
        ...
        for (size_t j = 0; j < input.size(); j += gradDim2nd) {
          ...
          for (size_t i = 0; i < output.size(); i += gradDim1st) {
            ...
            for (size_t k = 0; k < input.size(); k += 1) {
              ...
              tape.gradient(input[k].getIdentifier()) = GradientValue();
            }
    ...
    private:
      ...
      static CODI_INLINE void setGradientOnIdentifier(Tape& tape, size_t const pos, Identifier const* values, size_t const size, T value) {
        ...
        for (size_t curDim = 0; curDim < gradDim && pos + curDim < size; curDim += 1) {
          CODI_ENABLE_CHECK(ActiveChecks, tape.isActive(values[pos + curDim])) {
      ...
      template<typename T>
      static CODI_INLINE void setGradientOnCoDiValue(Tape& tape, size_t const pos, CoDiType* values, size_t const size, T value) {
        size_t constexpr gradDim = GT::dim();

        for (size_t curDim = 0; curDim < gradDim && pos + curDim < size; curDim += 1) {
          CODI_ENABLE_CHECK(ActiveChecks, tape.isActive(values[pos + curDim].getIdentifier())) {
      ...
      template<typename T>
      static CODI_INLINE void setGradient2ndOnCoDiValue(size_t const pos, CoDiType* values, size_t const size, T value) {
        using GT2nd = GradientValueTraits<typename Real::GradientValue>;
        size_t constexpr gradDim2nd = GT2nd::dim();
