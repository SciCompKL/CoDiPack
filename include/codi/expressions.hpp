...
#include "unaryExtendedExpressions.hpp"
...
namespace codi {
  ...
  /**
   * @brief Interface for extended unary elementary operation logic.
   *
   * @tparam Real   The real type used in the active types.
   *
   * Must be implemented for every unary elementary operation.
   * Each implementation is followed by an include of unaryExtendedExpression.tpp with prior defines.
   */
  template<typename Real>
  struct UnaryExtendedOpInterface {
    /**
     * @brief primal Primal function call.
     *
     * @param[in] a  The argument of the operation.
     ...
     */
    template<typename B>
    static CODI_INLINE Real primal(const Real& a, B b);
     ...
     * @tparam ReturnType  Indicates that the return type depends on the specific elementary operation.
     */
    template<typename ReturnType, typename B>
    static CODI_INLINE ReturnType gradient(const Real& a, B b, const Real& result);

    template<typename B>
    static CODI_INLINE double argToDouble(B b);

    template<typename B>
    static CODI_INLINE B doubleToArg(double const& d);
  };

  /**
   * @brief Operation logic for f(a,b) = ldexp(a, b)
   */
  using std::ldexp;
  template<typename Real>
  struct Ldexp : public UnaryExtendedOpInterface<Real> {
    /** \copydoc UnaryOpInterface::primal */
    static CODI_INLINE Real primal(const Real& a, int b) {
      return ldexp(a, b);
    }

    /** \copydoc UnaryOpInterface::gradient */
    static CODI_INLINE Real gradient(const Real& a, int b, const Real& result) {
      CODI_UNUSED(a);
      CODI_UNUSED(result);
      return ldexp(1.0, b);
    }

    static CODI_INLINE int doubleToArg(double const& d) {
      return (int)d;
    }

    static CODI_INLINE double argToDouble(int const& d) {
      return (double)d;
    }
  };
  #define OPERATION_LOGIC Ldexp
  #define FUNCTION ldexp
  #define ARG_TYPE int
  #include "unaryExtendedOverloads.tpp"

  /**
   * @brief Operation logic for f(a,b) = frexp(a, b)
   */
  using std::frexp;
  template<typename Real>
  struct Frexp : public UnaryExtendedOpInterface<Real> {
    /** \copydoc UnaryOpInterface::primal */
    static CODI_INLINE Real primal(const Real& a, int* b) {
      return frexp(a, b);
    }
    ...
    /** \copydoc UnaryOpInterface::gradient */
    static CODI_INLINE Real gradient(const Real& a, int const* b, const Real& result) {
      CODI_UNUSED(a);
      CODI_UNUSED(result);
