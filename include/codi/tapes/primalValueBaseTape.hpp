...
namespace codi {
  ...
  struct PrimalValueBaseTape : public CommonTapeImplementation<T_TapeTypes, T_Impl>,
    ...
    public:
      ...
      using VectorAccess = PrimalAdjointVectorAccess<Real, Identifier, Adjoint, AdjointVector>;
    ...
    protected:
      ...
      template<typename Adjoint, typename AdjointVector>
      ADJOINT_VECTOR_TYPE* selectAdjointVector(VectorAccess<Adjoint, AdjointVector>* vectorAccess, AdjointVector data) {
      ...
      template<bool copyPrimal, typename Adjoint, typename AdjointVector>
      CODI_NO_INLINE void internalEvaluateForward(Position const& start, Position const& end, AdjointVector data) {
        CODI_STATIC_ASSERT(
            Config::VariableAdjointInterfaceInPrimalTapes || CODI_T(std::is_same<AdjointVector, Adjoint*>::value),
            "Please enable 'CODI_VariableAdjointInterfaceInPrimalTapes' in order"
            " to use custom adjoint vectors in the primal value tapes.");
        ...
        VectorAccess<Adjoint, AdjointVector> vectorAccess(data, primalData);

        ADJOINT_VECTOR_TYPE* dataVector = selectAdjointVector<Adjoint, AdjointVector>(&vectorAccess, data);
    ...
    public:
      ...
      CODI_INLINE void evaluate(Position const& start, Position const& end, AdjointVector data) {
        internalEvaluateReverse<!TapeTypes::IsLinearIndexHandler, Adjoint, AdjointVector>(start, end, data);
      }
      ...
      CODI_INLINE void evaluateForward(Position const& start, Position const& end, AdjointVector data) {
        internalEvaluateForward<!TapeTypes::IsLinearIndexHandler, Adjoint, AdjointVector>(start, end, data);
      }
      ...
      VectorAccess<Gradient, Gradient*>* createVectorAccess() {
        return createVectorAccessCustomAdjoints<Gradient, Gradient*>(adjoints.data());
      }

      /// \copydoc codi::DataManagementTapeInterface::createVectorAccessCustomAdjoints()
      template<typename Adjoint, typename AdjointVector>
      VectorAccess<Adjoint, AdjointVector>* createVectorAccessCustomAdjoints(AdjointVector data) {
        return new VectorAccess<Adjoint, AdjointVector>(data, primals.data());
      }
      ...
      void evaluateForward(Position const& start, Position const& end,
        ...
        cast().template evaluateForward<Gradient, Gradient*>(start, end, adjoints.data());
      }
    ...
    public:
      ...
      CODI_INLINE void evaluate(Position const& start, Position const& end,
        ...
        evaluate<Gradient, Gradient*>(start, end, adjoints.data());
      }
    ...
    public:
      ...
      CODI_NO_INLINE void evaluatePrimal(Position const& start, Position const& end) {
        ...
        PrimalAdjointVectorAccess<Real, Identifier, Gradient, Gradient*> primalAdjointAccess(adjoints.data(),
                                                                                             primals.data());
