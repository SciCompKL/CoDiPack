  ...
  struct PrimalValueReuseTape : public PrimalValueBaseTape<T_TapeTypes, PrimalValueReuseTape<T_TapeTypes>> {
    ...
    protected:
      ...
      CODI_INLINE static void internalEvaluateForward_EvalStatements(
        ...
        CODI_UNUSED(endOtherDynamicDataPos, endOtherFixedDataPos, endConstantPos, endPassivePos, endRhsIdentifiersPos);
      ...
      CODI_INLINE void internalResetPrimalValues(Position const& pos) {
        ...
        using StmtPosition = typename StatementData::Position;
        StmtPosition startStmt = this->otherDynamicData.template extractPosition<StmtPosition>(this->getPosition());
        StmtPosition endStmt = this->otherDynamicData.template extractPosition<StmtPosition>(pos);
