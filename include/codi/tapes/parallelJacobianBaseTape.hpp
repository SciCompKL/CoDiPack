...
namespace codi {
  ...
  struct ParallelJacobianBaseTape : public CommonJacobianTapeImplementation<T_TapeTypes, T_Impl> {
    ...
    protected:
      ...
      static std::vector<Gradient> adjoints;  ///< Evaluation vector for AD.
    ...
    public:
      ...
      CODI_INLINE void swap(Impl& other) {
        // adjoints are shared and there is no need to swap them
  ...
  template<typename TapeTypes, typename ParallelToolbox, typename Impl>
  std::vector<typename TapeTypes::Gradient> ParallelJacobianBaseTape<TapeTypes, ParallelToolbox, Impl>::adjoints(1);

  template<typename TapeTypes, typename ParallelToolbox, typename Impl>
  typename ParallelToolbox::ReadWriteMutex ParallelJacobianBaseTape<TapeTypes, ParallelToolbox, Impl>::adjointsMutex;
}
